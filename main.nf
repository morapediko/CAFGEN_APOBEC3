...
process removeQCPhase1 {
  ...
  script:
     ...
     plink $K --autosome --bfile $base $sexinfo --geno 0.01 --make-bed --out temp1
     plink $K --autosome --bfile temp1 $sexinfo --mind 0.02 --make-bed --out temp1b
     plink $K --bfile temp1b  $sexinfo --geno $params.cut_geno --make-bed --out temp2
     /bin/rm temp1.{bim,fam,bed}
     plink $K --bfile temp2  $sexinfo --mind $params.cut_mind --make-bed --out temp3
...
process compPCA {
   ...
   script:
     ...
     """
     plink --bfile ${base} --indep-pairwise 1000 100 0.2 --out check
...
if (params.high_ld_regions_fname != "") {
  ...
  process pruneForIBDLD {
    ...
    script:
      ...
      """
       plink --bfile $base --threads $max_plink_cores --autosome $sexinfo $range --indep-pairwise 1000 100 0.2 --out ibd
  ...
  process pruneForIBD {
    ...
    script:
      ...
      outf   =  base.replace(".","_")
      """
       plink --bfile $base --threads $max_plink_cores --autosome $sexinfo  --indep-pairwise 1000 100 0.2 --out ibd
       #plink --bfile $base --threads $max_plink_cores --autosome $sexinfo --extract ibd.prune.in --genome --out ibd_prune
       plink --bfile $base --threads $max_plink_cores --autosome $sexinfo --extract ibd.prune.in --genome --min $pi_hat --out $outf
       echo NO_LD
       """
